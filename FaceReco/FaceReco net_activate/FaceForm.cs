...
namespace YunZhiFaceReco {
    public partial class FaceForm : Form {
        ...
        private List<byte[]> featureListFromDB = new List<byte[]>();

        private List<int> comparedResults = new List<int>();

        private int firstComparedResult;

        private int comparedTimes = 0;
        ...
        public FaceForm() {
            ...
            // debug
            MessageBox.Show("FaceForm，初始化成功");
            ...
            // debug
            MessageBox.Show("FaceForm，摄像头filterInfoCollection.Count = " + filterInfoCollection.Count + "，非0为正常");
            if (videoSource.IsRunning)//  摄像头未开启状态
            {
                // debug
                MessageBox.Show("FaceForm，摄像头未开启");
            ...
            {
                // debug
                MessageBox.Show("FaceForm，摄像头开启");
                if (isCompare) {
                    //比对结果清除
                ...
                // debug
                MessageBox.Show("FaceForm，摄像头开启状态下，videoSource \n" + videoSource);
                videoSource.Start();// 摄像头视频框启动
                // debug
                MessageBox.Show("FaceForm，摄像头开启状态下，videoSource.Start()");
            }
            /* ***********************************窗体启动自动打开摄像头视频框 end *************************************************  */
        ...
        private void InitEngines() {
            ...
            //debug
            MessageBox.Show("Activate Result:" + retCode);
            ...
            // debug
            MessageBox.Show("FaceForm，初始化引擎，引擎初始化结果:" + retCode + "，0为成功");
            ...
            // debug
            MessageBox.Show("FaceForm，初始化视频模式下人脸检测引擎，人脸检测引擎初始化结果:" + retCode + "，0为成功");
            ...
            // debug
            MessageBox.Show("FaceForm，初始化视频专用FR引擎，FR引擎初始化结果:" + retCode + "，0为成功");
        ...
        private void notifyIcon_MouseClick(object sender, MouseEventArgs e) {
            switch (e.Button) {
                ...
                case System.Windows.Forms.MouseButtons.Right:
                    // threadLockScreen.Start();
                    // Console.WriteLine("锁屏");
                    //MessageBox.Show("右键");
                    // Thread thread1 = new Thread(new ThreadStart(LockScreen));
                    //thread1.Start();
                    break;
            }
        }
        ...
        /// 锁屏按钮
        /// </summary>
        private void button1_Click_Th() {
            DialogResult dg = MessageBox.Show("确定锁屏么", "确定", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (dg == DialogResult.Yes) {
                if (textBoxTempLockPwd.Text == "")// 确保临时密码框不为空
                {
                    MessageBox.Show("您已被识别，请输入临时密码，才能锁屏！");
                }
                else {
                    LockScreenForm lockScreenForm = new LockScreenForm();
                    lockScreenForm.password = textBoxTempLockPwd.Text;
                    lockScreenForm.ShowDialog();
                    //textBoxTempLockPwd.Text = "ppman";
                }
        ...
        private void button1_Click_1(object sender, EventArgs e) {
            Thread lockThread = new Thread(button1_Click_Th);
